...
class EptdepartmentController extends Controller
{
  public function index(){
        ...
        // : DEFINE CHART MORRIS BAR
        return view('isept.adminept.eptdepartment')->with([
      			'homepage'         => $homepage,
  ...
  public function selectfaculty(Request $req){
    ...
    if ($req->ept_faculty == "" || ($req->ept_date_start == "" || $req->ept_date_end == "") || $req->ept_score == "") {
      return redirect()->route('adminept.eptdepartment')->with('error', 'Fakultas, Date Range, dan Date Skor wajib diisi');
    }
  ...
  if (($searchdate != null || $searchdate != "") && ($searchdate_end != null || $searchdate_end != "")) {
    $searchdate_data_score = " AND (es.created_at BETWEEN '".$searchdate."' AND '".$searchdate_end."') ";
    ...
    if ($search_score == 1) {
      ...
      $searchscore_data = " AND es.total_score IS NOT NULL";
              ...
              (
                ...
                LEFT JOIN ept as ept_1 ON reg.id_ept = ept_1.id
                WHERE ept_1.ept_date = ept.ept_date
                AND faculty.faculty_name = '".$searchfaculty."'
                ".$searchdate_data_y.$searchscore_data."
  ...
  public function pdf(Request $req){
    ...
    $pdf = PDFBarry::loadView("isept.adminept.pdfdepartment", [
  ...
  private function get_data($searchfaculty = null, $searchtype = null, $searchdate = null, $searchdate_end = null, $searchscore = null) {
    ...
    if ($searchfaculty != null) {
      ...
      $data = DB::select("SELECT * FROM
                ( SELECT majj.major_name as y,
                  ...
                  INNER JOIN ept as ept ON reg.id_ept = ept.id
                  WHERE faculty.id = ".$searchfaculty.$searchdate_data."
                  GROUP BY y
                  ORDER BY y ASC
    ...
    //    if ($searchtype == 1) {
    //      if (strpos($value->y, "S2") !== false || strpos($value->y, "S3") !== false) {
            // dd("S2 atau S3 ".$value->y);
    //        unset($data[$key]);
    //      }
    //    } elseif ($searchtype == 2) {
    //      if (strpos($value->y, "S1") !== false || strpos($value->y, "D3") !== false) {
            // dd("S1 atau D3 ".$value->y);
    //        unset($data[$key]);
    //      }
    //    }
      }

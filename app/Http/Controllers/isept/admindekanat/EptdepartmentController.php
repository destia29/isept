...
class EptdepartmentController extends Controller
{
  public function index(){
        ...
        $getAlias = $alias[2];
        $faculty = DB::table("faculty")->select("*")->where("faculty_alias", $getAlias)->first();
        $selectFaculty = DB::table("major")->select("*")->where("id_faculty", $faculty->id)->get();
        // dd($selectFaculty);
        ...
        return view('isept.admindekanat.eptdepartment')->with([
      			'homepage'         => $homepage,
  ...
  public function pdf(Request $req){
    ...
    if (($req->ept_date_start == "" && $req->ept_date_end == "") || $req->ept_score == "") {
      return redirect()->route('admindekanat.eptdepartment')->with('error', 'Date Range, dan Date Skor wajib diisi');
    }
    ...
    $pdf = PDFBarry::loadView("isept.admindekanat.pdfdepartment", [
      'ept_s1_raw'        => $ept_s1,
      'ept_s1'            => json_encode($ept_s1, JSON_NUMERIC_CHECK),
      'batch'             => $data,
      'jumlah_batch'      => count($data),
    ...
    $fileName = "EPT DEPARTEMENT DEKANAT ".date("Y-m-d");
    ...
    if ($searchfaculty != null) {
      ...
      $data = DB::select("SELECT * FROM
                ( SELECT majj.major_name as y,
                  ...
                  GROUP BY y
                  ORDER BY y ASC
                ) as ept_s1 ORDER BY ept_date ASC, ept_s1.y ASC
      ");
